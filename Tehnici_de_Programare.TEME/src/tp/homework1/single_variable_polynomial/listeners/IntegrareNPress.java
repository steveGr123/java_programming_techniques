 ...
 * nedefinita. Constructorul va lua ca parametrii fereastra de input in care s-a
 ...
 * 
 */
public class IntegrareNPress implements MouseListener {
	private InputWindow in;
	private OutputWindow out;
	private PlotWindow outP;

	public IntegrareNPress(InputWindow inputWindow, OutputWindow outputWindow,
			PlotWindow outP) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		final Polinom rezultat = p.toPolinom(
				in.getTextFieldPolinom1().getText()).integrare();
		for (int i = 0; i < yAxis.length; i++) {
			yAxis[i] = rezultat.evaluare(i - 15);
			xAxis[i] = i - 15;
		}
		String htmlModifier = rezultat.toString();
